...
fun ToDayTasksScreen(navController: NavHostController, viewModel: TasksScreenViewModel) {
    ...
    if (change || change.not()) {
        Column {
            ...
            LazyColumn {
                items(tasks.filter { task -> !task.isComplete && filterToDay(task) }) {
                    TaskItem(
                        task = it,
                ...
                item {
                    if (tasks.filter {task -> task.isComplete && filterToDay(task) }.isNotEmpty()
                        && tasks.filter {task -> !task.isComplete && filterToDay(task) }.isNotEmpty()){
                        Divider(
                            modifier = Modifier
